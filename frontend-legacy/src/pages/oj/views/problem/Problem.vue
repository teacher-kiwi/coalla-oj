...
export default {
  ...
  data() {
    return {
      ...
      submissionId: "",
      submitted: false,
      blocklyWorkspace: "", // Store Blockly workspace XML
      result: {
        result: 9,
      },
      problem: {
  ...
  beforeRouteEnter(to, from, next) {
    ...
    if (problemCode) {
      next((vm) => {
        ...
        if (problemCode.blocklyState) {
          vm.blocklyWorkspace = problemCode.blocklyState;
        }
      });
    } else {
      next();
  ...
  methods: {
    ...
    init() {
      ...
      api[func](this.problemID, this.contestID).then(
        (res) => {
          ...
          // If Python3 is available, default to Block Coding, otherwise use first language
          if (this.problem.languages.includes("Python3")) {
            this.language = "Block Coding";
          } else {
    ...
    onWorkspaceChanged(workspaceJson) {
      this.blocklyWorkspace = workspaceJson;
    },
    ...
    submitCode() {
      ...
      let data = {
        ...
        blockly_state:
          this.language === "Block Coding" ? this.blocklyWorkspace : "",
      };
